...
def split_image_random(img, ext):
    """
    이미지를 내부적으로 Grid(10~20행, 10~20열)로 나눈 후,
    겹침/누락 없이 무작위 병합을 통해 랜덤 조각을 생성하여 pieces 디렉토리에 저장합니다.
    num_pieces는 사용하지 않습니다.
    """
    ...
    def can_merge(r, c, dr, dc):
        nr, nc = r + dr, c + dc
        if 0 <= nr < rows and 0 <= nc < cols:
            return not used[nr][nc]
        return False
    ...
    for r, c in grid_coords:
        ...
        merge_type = random.choice(["right", "down", "none"])  # 병합 방향
        coords = [(r, c)]
        used[r][c] = True

        if merge_type == "right" and can_merge(r, c, 0, 1):
            coords.append((r, c + 1))
            used[r][c + 1] = True
        elif merge_type == "down" and can_merge(r, c, 1, 0):
            coords.append((r + 1, c))
            used[r + 1][c] = True

        # 조각 생성
        top = min(y for y, x in coords)
        left = min(x for y, x in coords)
        bottom = max(y for y, x in coords)
        right = max(x for y, x in coords)

        x1, y1 = left * cell_w, top * cell_h
        x2 = (right + 1) * cell_w if right + 1 < cols else width
        y2 = (bottom + 1) * cell_h if bottom + 1 < rows else height

        piece = img[y1:y2, x1:x2]
        filename = os.path.join(pieces_dir, f"{uuid.uuid4().hex[:8]}.{ext}")
        cv2.imwrite(filename, piece)
        count += 1

    print(f"총 {count}개의 랜덤 병합 조각 생성 완료 (Grid: {rows}x{cols})")
...
def perform_split():
    ...
    if method == "grid":
        ...
        # split_image_random(original_image, num_pieces, ext)
